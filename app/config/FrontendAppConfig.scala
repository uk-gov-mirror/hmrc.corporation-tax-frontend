...
class FrontendAppConfig @Inject()(config: ServicesConfig,
  ...
  private lazy val tarHost: String = config.getString("tax-account-router-frontend.host")
  ...
  lazy val isUrBannerEnabled: Boolean = config.getBoolean("toggles.urBannerEnabled")
}
